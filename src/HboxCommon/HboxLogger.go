...
const (
	HBOX_LOGGER_LEVEL_FATAL   uint16 = 0
	HBOX_LOGGER_LEVEL_ERROR   uint16 = 1
	HBOX_LOGGER_LEVEL_WARNING uint16 = 2
	HBOX_LOGGER_LEVEL_INFO    uint16 = 3
	HBOX_LOGGER_LEVEL_DEBUG   uint16 = 4
	HBOX_LOG_FILE_SIZE        int64  = 10 * 1024 * 1024
)
...
func (l *Logger) Init(fileName string) {
	...
	l.fileName = fileName
	l.logLevel = HBOX_LOGGER_LEVEL_ERROR
	...
	go func() {
		for {
			line := <-l.LogCh
			switch line.Level {
			case HBOX_LOGGER_LEVEL_DEBUG:
				l.LogDebug.Println(line.LogLine)
			case HBOX_LOGGER_LEVEL_INFO:
				l.LogInfo.Println(line.LogLine)
			case HBOX_LOGGER_LEVEL_WARNING:
				l.LogWarning.Println(line.LogLine)
			case HBOX_LOGGER_LEVEL_ERROR:
				l.LogError.Println(line.LogLine)
			case HBOX_LOGGER_LEVEL_FATAL:
...
func (l *Logger) SetLoglevel(level string) {
	...
	switch level {
	case "DEBUG":
		levelInt = HBOX_LOGGER_LEVEL_DEBUG
	case "INFO":
		levelInt = HBOX_LOGGER_LEVEL_INFO
	case "ERROR":
		levelInt = HBOX_LOGGER_LEVEL_ERROR
	case "WARNING":
		levelInt = HBOX_LOGGER_LEVEL_WARNING
	case "FATAL":
		levelInt = HBOX_LOGGER_LEVEL_FATAL
	default:
		levelInt = HBOX_LOGGER_LEVEL_FATAL
	}
	l.logLevel = levelInt
...
func (l *Logger) Fatal(args ...interface{}) {
	...
	l.LogCh <- LogLine{line, HBOX_LOGGER_LEVEL_FATAL}
}
...
func (l *Logger) Errorf(format string, args ...interface{}) {
	...
	if l.logLevel < HBOX_LOGGER_LEVEL_ERROR {
		return
	}

	fileName, lineno := l.GetCaller()
	line := fmt.Sprintf("%s:%d "+format, fileName, lineno, args)
	l.LogCh <- LogLine{line, HBOX_LOGGER_LEVEL_ERROR}
}
...
func (l *Logger) Warningf(format string, args ...interface{}) {
	if l.logLevel < HBOX_LOGGER_LEVEL_WARNING {
		return
	}

	fileName, lineno := l.GetCaller()
	line := fmt.Sprintf("%s:%d "+format, fileName, lineno, args)
	l.LogCh <- LogLine{line, HBOX_LOGGER_LEVEL_WARNING}
}
...
func (l *Logger) Infof(format string, args ...interface{}) {
	if l.logLevel < HBOX_LOGGER_LEVEL_INFO {
		return
	}

	fileName, lineno := l.GetCaller()
	line := fmt.Sprintf("%s:%d "+format, fileName, lineno, args)
	l.LogCh <- LogLine{line, HBOX_LOGGER_LEVEL_INFO}
}
...
func (l *Logger) Debugf(format string, args ...interface{}) {
	if l.logLevel < HBOX_LOGGER_LEVEL_DEBUG {
		return
	}

	fileName, lineno := l.GetCaller()
	line := fmt.Sprintf("%s:%d "+format, fileName, lineno, args)
	l.LogCh <- LogLine{line, HBOX_LOGGER_LEVEL_DEBUG}
}
...
func (l *Logger) CheckSize() {
	...
	if fileInfo.Size() > HBOX_LOG_FILE_SIZE {
		l.gotoNextVersion()
	}
}
